...
const handleGeminiError = (error: any) => {
  ...
  const isExpired = errorMessage.includes("expired") || error?.status === "INVALID_ARGUMENT" && errorMessage.includes("key");
  
  if (errorMessage.includes("leaked") || isExpired || error?.status === "PERMISSION_DENIED") {
    throw new Error(`API_KEY_ISSUE: Your Gemini API key ${isExpired ? "has expired" : "has been reported as leaked or is invalid"}. Please update it in the Settings > Secrets menu to continue using AI features.`);
  }
  
  if (error?.status === "RESOURCE_EXHAUSTED" || errorMessage.includes("quota")) {
    throw new Error("QUOTA_ISSUE: AI quota exhausted. Please try again later or upgrade to a paid tier in Settings > Secrets.");
  }
...
export const extractLogFromAudio = async (
  ...
  try {
    ...
    const systemInstruction = `
      ...
      - Capture the exact details of the media or techniques mentioned using the artist's own descriptions.
    `;
    ...
    try {
      ...
      throw new Error("Could not extract structured data. Please try again or use manual entry.");
    }
  } catch (err) {
    handleGeminiError(err);
    throw err; // Should not reach here due to handleGeminiError throwing
  }
};
...
export const summarizeSeed = async (base64Audio: string, mimeType: string): Promise<string | null> => {
  try {
    ...
    // For seeds we don't want to throw and crash the live harvest, just log
    // We still want to see the clear error in the console
    const errorMessage = err?.message?.toLowerCase() || '';
    const isExpired = errorMessage.includes("expired") || err?.status === "INVALID_ARGUMENT" && errorMessage.includes("key");
    const isLeaked = errorMessage.includes("leaked") || err?.status === "PERMISSION_DENIED";

    if (isExpired || isLeaked) {
      console.error(`Gemini API Key Issue during seed harvesting: ${isExpired ? "EXPIRED" : "INVALID/LEAKED"}. Please check Secrets.`);
    } else {
      console.error("Seed harvesting failed:", err);
    }
    return null;
  }
};
